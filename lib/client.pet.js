/*
 * dsh-pixel-liangzu — DSH browser/client half.
 *
 ...
 * `/plugins/dsh-pixel-liangzu/gifs/{01-06}_{idle,rest,work,done,wait}.gif`.
 */
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const PLUGIN_ID = 'dsh-pixel-liangzu';
    const NS = 'dshpixelpet.pet';
    const GIF_URL_PREFIX = '/plugins/dsh-pixel-liangzu/gifs';
    ...
    const POSITION_KEY = 'dsh-pixel-liangzu.position.v1';
    const POSITION_EDGE = 8;

    function readStoredPosition() {
      try {
        const raw = window.localStorage.getItem(POSITION_KEY)
          || window.localStorage.getItem('dsh-pixel-pet.position.v1');
        if (raw) {
...
`;
    ...
    function apply(ctx) {
      ctx.effect(() => ctx.locale.register(NS, { zh, en }), 'dsh-pixel-liangzu: dictionaries');

      ctx.effect(() => {
        const removeStyles = installStyles();
        return removeStyles;
      }, 'dsh-pixel-liangzu: styles');
